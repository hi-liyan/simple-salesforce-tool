...
import { ArrowLeftRight, Braces, ChevronLeft, Clipboard, RotateCcw, Sparkles, WandSparkles } from "lucide-react";
...
import { buildJsonDiffSemanticState } from "../logic/jsonDiff";
import { JsonDiffTab, useJsonDiffStore } from "../../../../store/useJsonDiffStore";

type JsonDiffToolProps = {
  // 返回工具入口页：用于离开 JSON Diff 工具详情页。
  onBack: () => void;
};

// Diff 编辑器实例类型：用于访问左右编辑器与模型。
type DiffEditorInstance = Monaco.editor.IStandaloneDiffEditor;

// 示例左侧 JSON：用于快速体验语义比较能力。
const DEFAULT_LEFT_SAMPLE_JSON = `{
  "name": "simple-salesforce-tool",
  "meta": {
    "enabled": true,
    "version": 1
  },
  "features": ["query", "terminal", "json-diff"]
}`;

// 示例右侧 JSON：刻意调整键顺序与字段值，便于观察文本差异和语义状态。
const DEFAULT_RIGHT_SAMPLE_JSON = `{
  "features": ["query", "terminal", "json-diff", "json-formatter"],
  "meta": {
    "version": 2,
    "enabled": true
  },
...
// JSON Diff 页签内容：负责左右输入、语义状态与差异展示。
function JsonDiffTabPane({
  tab,
  active,
  onPatchTab,
  onShowNotice
  ...
  active: boolean;
  // 更新当前页签。
  onPatchTab: (tabId: string, updater: (current: JsonDiffTab) => JsonDiffTab) => void;
  // 展示顶部提示条。
  ...
  // 延迟左侧文本：降低高频输入时的同步解析抖动。
  const deferredLeftText = useDeferredValue(tab.leftText);
  // 延迟右侧文本：降低高频输入时的同步解析抖动。
  const deferredRightText = useDeferredValue(tab.rightText);

  // 语义状态：统一提供空态、错误态、可比较性与一致性。
  const semanticState = useMemo(() => buildJsonDiffSemanticState(deferredLeftText, deferredRightText), [deferredLeftText, deferredRightText]);
  ...
  const summary = useMemo(() => {
    ...
    const hasError = Boolean(semanticState.left.errorMessage || semanticState.right.errorMessage);
    const statusText = semanticState.empty ? "等待输入" : hasError ? "JSON 输入有误" : semanticState.semanticallyEqual ? "语义一致" : "语义差异";
    const statusClassName = semanticState.empty
      ? "text-neutral/55"
      : hasError
        ? "text-error"
        : semanticState.semanticallyEqual
          ? "text-success"
          : "text-primary";
  ...
  }, [deferredLeftText, deferredRightText, semanticState]);
  ...
  // 将左右文本同步到 Monaco 模型：仅在外部动作改变内容时触发，避免击键时重置光标。
  function syncEditorModels(nextLeftText: string, nextRightText: string) {
    ...
    try {
      if (originalModel.getValue() !== nextLeftText) {
        originalEditor.executeEdits("json-diff-sync-left", [
          {
      ...
      if (modifiedModel.getValue() !== nextRightText) {
        modifiedEditor.executeEdits("json-diff-sync-right", [
          {
  ...
  function bindModelListeners(editor: DiffEditorInstance) {
    ...
    if (originalModel) {
      originalListenerRef.current = originalModel.onDidChangeContent(() => {
        ...
        leftTextRef.current = nextValue; // 先更新本地 ref，避免 store 回流时触发误判。
        onPatchTab(tab.id, (current) => ({
          ...current,
          leftText: nextValue
        }));
    ...
    if (modifiedModel) {
      modifiedListenerRef.current = modifiedModel.onDidChangeContent(() => {
        ...
        rightTextRef.current = nextValue; // 先更新本地 ref，避免 store 回流时触发误判。
        onPatchTab(tab.id, (current) => ({
          ...current,
          rightText: nextValue
        }));
  ...
  // 载入示例 JSON：便于快速体验语义比较与高亮差异。
  function loadSampleJson() {
    applyTabTexts(DEFAULT_LEFT_SAMPLE_JSON, DEFAULT_RIGHT_SAMPLE_JSON);
    onShowNotice(null);
  }
  ...
  function swapTexts() {
    ...
    onShowNotice({
      tone: "success",
      message: `“${tab.name}” 已交换左右 JSON。`
    });
  }

  // 按语义格式化左右 JSON：解析成功后按稳定键顺序统一输出。
  function normalizeJsonBySemantic() {
    if (semanticState.left.errorMessage || semanticState.right.errorMessage) {
      onShowNotice({
        tone: "warning",
        message: "存在 JSON 解析错误，无法执行语义格式化。请先修正错误后重试。"
      });
      return;
    }

    const nextLeftText = semanticState.left.normalizedText || "";
    const nextRightText = semanticState.right.normalizedText || "";
    applyTabTexts(nextLeftText, nextRightText);
    onShowNotice({
      tone: "success",
      message: `“${tab.name}” 已按 JSON 语义完成格式化。`
    });
  }
  ...
  async function handleCopyText(side: "left" | "right") {
    ...
    if (!targetText.trim()) {
      onShowNotice({
        tone: "warning",
        message: side === "left" ? "左侧当前没有可复制的 JSON 文本。" : "右侧当前没有可复制的 JSON 文本。"
      });
      return;
    }
  ...
  return (
    // 单个 JSON Diff 页签：激活时展示，不激活时仅隐藏保留编辑器状态。
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden p-4">
        ...
        <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-base-300 bg-base-100 px-4 py-3 shadow-sm">
            ...
            {/* 摘要状态：根据空态/错误态/一致性给出明确反馈。 */}
            <p className={`mt-2 text-[16px] font-semibold ${summary.statusClassName}`}>{summary.statusText}</p>
          </div>
        </section>
        {/* 主体卡片：顶部工具栏 + 下方双栏 JSON diff 编辑器。 */}
        <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-base-300 bg-base-100 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-base-300 px-4 py-3">
            <div>
              {/* 卡片标题。 */}
              <h3 className="text-[14px] font-semibold text-neutral">双栏 JSON 对比</h3>
            </div>
            {/* 功能按钮区：统一收纳常用操作。 */}
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <button type="button" className="btn btn-ghost btn-sm h-8 min-h-8 gap-2 px-3 text-[12px]" onClick={normalizeJsonBySemantic}>
          ...
          {/* 错误提示区：分别显示左右 JSON 解析错误，便于快速定位。 */}
          {semanticState.left.errorMessage || semanticState.right.errorMessage ? (
            <div className="grid grid-cols-1 gap-2 border-b border-base-300 bg-base-100/80 px-4 py-3 md:grid-cols-2">
              {semanticState.left.errorMessage ? <NoticeAlert tone="error" message={`左侧：${semanticState.left.errorMessage}`} /> : <div />}
              {semanticState.right.errorMessage ? <NoticeAlert tone="error" message={`右侧：${semanticState.right.errorMessage}`} /> : <div />}
            </div>
          ) : null}
          {/* Diff 编辑器区域：始终展示双栏编辑器，空白时额外给出输入引导。 */}
          <div className="min-h-0 flex-1 bg-[linear-gradient(180deg,#fbfdff_0%,#f3f8ff_100%)] p-4">
            <div className="flex h-full min-h-0 flex-col gap-3">
              {semanticState.empty ? (
                // 空白提示：提醒用户可直接在左右栏输入 JSON。
                <div className="flex items-start gap-3 rounded-2xl border border-dashed border-primary/25 bg-base-100/70 px-4 py-3">
                  ...
                  <div className="min-w-0">
                    {/* 空白提示标题。 */}
                    <p className="text-[13px] font-medium text-neutral">JSON 编辑器已就绪</p>
                    {/* 空白提示说明。 */}
                    <p className="mt-1 text-[12px] leading-6 text-neutral/60">请在左右两栏输入 JSON 文本，系统会自动给出语义比较状态与差异高亮。</p>
                  </div>
                </div>
              ) : null}
              <div className="grid grid-cols-2 gap-3 text-[12px] text-neutral/60">
                {/* 左侧标签：明确当前左栏输入区域。 */}
                <div className="rounded-xl border border-base-300/80 bg-base-100/70 px-3 py-2">左侧 JSON</div>
                {/* 右侧标签：明确当前右栏输入区域。 */}
                <div className="rounded-xl border border-base-300/80 bg-base-100/70 px-3 py-2">右侧 JSON</div>
              </div>
              <div className="h-full overflow-hidden rounded-2xl border border-base-300 bg-base-100 shadow-inner">
                <DiffEditor
                  ...
                  language="json"
                  originalModelPath={`json-diff://${tab.id}/left`}
                  modifiedModelPath={`json-diff://${tab.id}/right`}
...
// JSON Diff 工具页：支持多 Tab、持久化与进入页面后懒恢复。
export function JsonDiffTool({ onBack }: JsonDiffToolProps) {
  // Store 状态：维护全部页签与当前激活项。
  const tabs = useJsonDiffStore((state) => state.tabs);
  const activeTabId = useJsonDiffStore((state) => state.activeTabId);
  const setActiveTabId = useJsonDiffStore((state) => state.setActiveTabId);
  const createTab = useJsonDiffStore((state) => state.createTab);
  const patchTab = useJsonDiffStore((state) => state.patchTab);
  const reorderTabs = useJsonDiffStore((state) => state.reorderTabs);
  const closeTab = useJsonDiffStore((state) => state.closeTab);
  const tabOrder = useJsonDiffStore((state) => state.tabOrder);

  // 工具级提示：承载复制、格式化等反馈。
  const [notice, setNotice] = useState<{ tone: NoticeTone; message: string } | null>(null);
  // 懒恢复完成标记：进入 JSON Diff 工具页后才触发持久化恢复。
  const [hydrated, setHydrated] = useState(useJsonDiffStore.persist.hasHydrated());
  ...
  useEffect(() => {
    ...
    void Promise.resolve(useJsonDiffStore.persist.rehydrate()).then(() => {
      const storeState = useJsonDiffStore.getState();
      ...
      <div className="flex h-full w-full items-center justify-center bg-base-200/40">
        <div className="rounded-2xl border border-base-300 bg-base-100 px-5 py-4 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              {/* 加载标题。 */}
              <p className="text-[13px] font-medium text-neutral">正在恢复 JSON Diff 工具页签</p>
              <p className="mt-1 text-[12px] text-neutral/65">这里只会在进入 JSON Diff 工具后按需恢复，不影响应用启动速度。</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    // JSON Diff 工具页：顶部返回 + Tab 栏 + 多页签工作区。
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-base-200/45">
      ...
      <div className="flex items-center justify-between gap-4 border-b border-base-300 bg-base-100 px-5 py-3">
        ...
        {/* 工具说明：强调语义比较与懒恢复行为。 */}
        <p className="text-[12px] text-neutral/60">支持多 Tab JSON 语义对比、双栏差异高亮与按需恢复。</p>
      </div>
      <ReusableTabs
        ...
        createButtonTitle="新建 JSON Diff 页签"
        ...
        onCloseTabs={(tabIds) => {
          useJsonDiffStore.getState().closeTabsByIds(tabIds);
          setNotice(null);
        }}
      />
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden">
        {mountedTabs.map((tab) => (
          <JsonDiffTabPane key={tab.id} tab={tab} active={tab.id === activeTab?.id} onPatchTab={patchTab} onShowNotice={setNotice} />
        ))}
      </div>
    </div>
  );
